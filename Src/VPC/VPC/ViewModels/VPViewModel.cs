...
public class VPViewModel : BindableBaseViewModel, IVPViewModel
{
  ...
  readonly DispatcherTimer _timer = new() { Interval = TimeSpan.FromSeconds(0.14) };
  ...
  readonly AmbienceLib.Bpr _bpr = new();
  ...
  public TimeSpan VMPosn
  {
    ...
    set
    {
      if (Set(ref _vmPosn, value))
      {
        ...
        {
          _prevPosn = value;
          _ = Task.Run(async () => await Task.Delay(125)).ContinueWith(_ =>
          {
            if (_prevPosn == value)
            {
              //Bpr.BeepOk();
  ...
  public VPModel VPModel { get; set; } = new VPModel();
  ...
  public bool IsPlaying { get => _IsBusy; set { _ = Set(ref _IsBusy, value); MUProgressState = value ? TaskbarItemProgressState.Normal : TaskbarItemProgressState.Paused; } }
  bool _IsBusy = false;
  ...
  {
    var dlgwin = new UsedMediaChoicesWindow
    {
      MediaFile = _vpcPlayer.Source.LocalPath,
      Owner = Window
    };     //dlgwin.Owner = ((FrameworkElement)(((FrameworkElement)(this)).Parent)).Parent as Window;
    _ = dlgwin.ShowDialog();
  ...
  void do_F2_(object o)
  {
    ...
    if (rv == true)
    {
      VPModel.CrntMU.Rename(w.Filename);
    }
  }
  ...
  void do_F4_(object o)
  {
    ...
    try
    {
      ...
      if (!string.IsNullOrEmpty(mf) && !File.Exists(mf))
      {
        ...
        _ = Process.Start(new ProcessStartInfo(dir));
        Bpr.BeepOk();
      }
      //else
  ...
  void do_D2_(object o)
  {
    ...
    {
      VPModel.CrntMU.OrgHttpLink = VPModel.CrntMU.OrgHttpLink = Clipboard.GetText();
      VPModel.CrntMU.SaveLink(VPModel.CrntMU.OrgHttpLink);
  ...
  {
    flashKeyInfo();
    try { _ = Process.Start(new ProcessStartInfo("Explorer", Path.GetDirectoryName(_vpcPlayer.Source.LocalPath))); } catch (Exception ex) { _ = ex.Log(); }
  }
  ...
  void do_M_(object o)
  {
    ...
    else _vpcPlayer.Height = _vpcPlayer.Height == 2 * _vpcPlayer.NaturalVideoHeight ? 3 * _vpcPlayer.NaturalVideoHeight : _vpcPlayer.NaturalVideoHeight;
  }
  ...
  //  while (parent is not Window) { parent = VisualTreeHelper.GetParent(parent); }
  //  return parent;
  //}
  ...
  void setSpeed() // needed setting to some other speed before it will comply with the new speed value (~2018)
  {
    ...
    _ = Task.Run    /**/(async () => await Task.Delay(delayMs)).ContinueWith(_ => _vpcPlayer.SpeedRatio = tmpSpeed, TaskScheduler.FromCurrentSynchronizationContext()).ContinueWith(_ => Trace.WriteLine($"** 1)                      {_vpcPlayer.SpeedRatio} <= {tmpSpeed}?"))
  ...
  void playUsingExternalPlayer(string? optExe = null)
  {
    ...
    if (exe == null)
    {
      _ = MessageBox.Show("No suitable players found");
    }
    else
    {
      ...
      }).ContinueWith(_ =>
      {
        ...
        if (externalPlayerViewTime.TotalSeconds is > 5 and < 7200)
        {
  ...
  void removeNearBookmarksInRangeOf(int sec)
  {
    ...
    {
      _ = VPModel.CrntMU.Bookmarks.Remove(bmk);
    }
  }
  ...
  public void PlayNewFile(string mediaFile)
  {
    ...
    {
      _ = Process.Start(VPModel.CrntMU.OrgHttpLink);
      return;
    }
